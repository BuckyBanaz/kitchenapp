...
class FinalizeOrderScreen extends StatefulWidget {
  const FinalizeOrderScreen({super.key});
...
class _FinalizeOrderScreenState extends State<FinalizeOrderScreen> {
  final BookOrderController controller = Get.put(BookOrderController());
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title:  Text(AppStrings.finalizeOrderTitle),
        leading: IconButton(
          ...
          return ListView(
            children: [
              ...
              CustomInputField(
                ...
                nextFocus: customer == null ? null : controller.personFocus,
                inputAction: TextInputAction.next,
              ),
              const SizedBox(height: 16),

              // Login if not customer
              if (customer == null) ...[
                Obx(
                  () => controller.isLoggingIn.value
                      ...
                      : CustomRedButton(
                          text: "Continue",
                          onPressed: controller.loginOnlyWithNamePhone,
                        ),
                ),
                const SizedBox(height: 16),
                const Text(
                  "Please continue to proceed with order.",
              ...
              if (customer != null) ...[
                CustomInputField(
                  ...
                  hintText:  AppStrings.hintextPerson,
                ...
                CustomInputField(
                  ...
                  hintText: AppStrings.enter_address,
                ...
                CustomInputField(
                  ...
                  onDone: controller.submitOrder,
                ),

                const SizedBox(height: 16),
                Text("• ${"orderConfirm".tr}"),
                Text("• ${"orderModify".tr}"),
                Text("• ${"deliveryCharge".tr}"),
                Text("• ${"pickupNote".tr}"),
                Text("• ${"dishCharge".tr}"),
                Text("• ${"dishCleanReturn".tr}"),
                Text("• ${"dishUncleanCharge".tr}"),
      ...
      bottomNavigationBar: Obx(() {
        // Show bottom bar only if customer exists
        if (controller.currentCustomer.value == null) return const SizedBox.shrink();
          ...
          child: controller.isLoading.value
            ...
            onPressed: () {
              Get.defaultDialog(
                ...
                title: AppStrings.confirm_order,
                middleText: AppStrings.confirm_msg,
                textCancel: AppStrings.no,
                textConfirm: AppStrings.yes,
                confirmTextColor: Colors.white,
                onConfirm: () {
                  Get.back(); // Close dialog
                  controller.submitOrder(); // Proceed with order
                },
              );
            },
          ),
